...
pub fn uno() {
    let mut valid: u32 = 0;
    for l in INPUT.iter() {
        let read = read_line(l);
        let o = read.password.matches(read.letter).count();
        if o >= read.min_count && o <= read.max_count {
            valid += 1;
        }
    }
    println!("{}", valid)
...
pub fn due() {
    ...
    for l in INPUT.iter() {
        let read = read_line(l);
        let c1 = read.password.chars().nth(read.min_count - 1).unwrap() == read.letter;
        // println!("len: {}, min: {}, max: {}", read.password.len(), read.min_count, read.max_count);
        let c2 = read.password.chars().nth(read.max_count - 1).unwrap() == read.letter;
...
fn read_line(input: &str) -> Line {
    let count = input.split(" ").nth(0).unwrap();
    let letter = input.split(" ").nth(1).unwrap().split(":").nth(0).unwrap().chars().next().unwrap();
    let password = input.split(" ").nth(2).unwrap().to_string();

    let min_count = count.split("-").nth(0).unwrap().parse::<usize>().unwrap();
    let max_count = count.split("-").nth(1).unwrap().parse::<usize>().unwrap();

    Line {min_count, max_count, letter, password}
}